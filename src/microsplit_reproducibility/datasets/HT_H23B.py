...
from careamics.lvae_training.dataset import DataSplitType, TilingMode
from careamics.lvae_training.dataset.utils.empty_patch_fetcher import EmptyPatchFetcher
from careamics.lvae_training.dataset.utils.index_manager import GridIndexManagerRef
...
class MultiChDloaderRef:
    def __init__(
        ...
        # Note: probably not true, bc input is weighted average of all channels
        ...
        self._is_validation = data_config.datasplit_type == DataSplitType.Val
        ...
        if self._is_train or self._is_validation:
        ...
        self._uncorrelated_channels = data_config.uncorrelated_channels

        self._uncorrelated_channel_probab = data_config.uncorrelated_channel_probab
        # _uncorrelated_channels should be False only for testing ?
        assert self._is_train or self._is_validation or self._uncorrelated_channels is False
    ...
    def compute_mean_std(self):
        ...
        for _ in range(30000):
            ...
            imgs = np.stack(crops, axis=0)  # C,Y,X
            inp = np.sum(imgs * alphas.reshape(-1, 1, 1), axis=0)
    ...
    def _compute_target(self, img_tuples):
    ...
    def _get_alpha(self) -> np.ndarray:
        if self._is_train or self._is_validation:
            alpha = [1 / self._num_channels for _ in range(self._num_channels)]
        else:
            # test data has already mixed
            alpha = [1 for _ in range(self._num_channels)]
    ...
    def _compute_input(self, img_tuples):
        ...
        elif self._is_train or self._is_validation:
            # get weighted average of channels
            alphas = self._get_alpha()
            imgs = np.stack(img_tuples, axis=0)  # C,Y,X
            inp = np.sum(imgs * alphas.reshape(-1, 1, 1), axis=0)
        else:
            # for test data: it's already mixed.
            inp = img_tuples[0]
    ...
    def _rotate(self, img_tuples):
        ...
        if self._3Ddata:
            return self._rotate3D(img_tuples)
        else:
    ...
    def __getitem__(
        ...
        if (
            (self._is_train or self._is_validation)
        ...
        # mixing channels and normalize
        inp = self._compute_input(input_tuples)

        target = self._compute_target(input_tuples)
